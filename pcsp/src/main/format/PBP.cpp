...
bool PBP::deleteDir(const std::string &path) {
    if (std::filesystem::is_directory(pbpUnpackPath)) {
        std::filesystem::remove_all(pbpUnpackPath);  // delete all files and directory
    }
    return true;
}

void PBP::unpackOne(const std::string &path, std::ifstream &f, u32 offset, u32 size) {
    if (size == 0) return;
...
void PBP::unpackPBP(std::ifstream &f) {
    ...
    std::filesystem::create_directory(pbpUnpackPath);
...
PBP::PBP(const PBP &other) { *this = other; }
...
void PBP::setInfo(const std::string &info) { this->info = info; }

Elf32 *PBP::getElf32() const { return elf32; }

PSF *PBP::getPsf() const { return psf; }

std::string PBP::getInfo() const { return info; }

u32 PBP::getMagic() const { return data.p_magic; }

u32 PBP::getVersion() const { return data.p_version; }

u32 PBP::getOffsetParam() const { return data.p_offset_param_sfo; }

u32 PBP::getOffsetIcon0() const { return data.p_offset_icon0_png; }

u32 PBP::getOffsetIcon1() const { return data.p_offset_icon1_pmf; }

u32 PBP::getOffsetPic0() const { return data.p_offset_pic0_png; }

u32 PBP::getOffsetPic1() const { return data.p_offset_pic1_png; }

u32 PBP::getOffsetSnd0() const { return data.p_offset_snd0_at3; }

u32 PBP::getOffsetPspData() const { return data.p_offset_psp_data; }

u32 PBP::getOffsetPsarData() const { return data.p_offset_psar_data; }

u32 PBP::getSizeIcon0() const { return size_icon0_png; }
